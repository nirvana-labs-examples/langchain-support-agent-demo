...
Datasets are committed as zip files (`data/small.zip`, `data/medium.zip`, `data/large.zip`) and extracted automatically on first use. To regenerate: `python -m scripts.generate_dataset`.
...
langchain-support-agent-demo/
  ...
  data/
    small.zip                      ← 24 KB dataset (auto-extracted on first use)
    medium.zip                     ← 10 MB dataset (auto-extracted on first use)
    large.zip                      ← 104 MB dataset (auto-extracted on first use)
